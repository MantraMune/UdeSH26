import NLPModels, NLPModels.obj, NLPModels.grad, NLPModels.grad!, NLPModels.hess
...
#:: nlp.NLPModelMeta
#:: nlp.Counters
#meta = nlp.NLPModelMeta(1, x0=zeros(1), name="LineModel to $(nlp.meta.name))")


function LineModel(nlp :: ADNLPModel, x :: Vector{Float64}, d :: Vector{Float64})
  meta = NLPModels.NLPModelMeta(1, x0=zeros(1), name="LineModel to $(nlp.meta.name))")
  counters = NLPModels.Counters()
  return LineModel(nlp, meta, counters, x, d)
end
...
function obj(f :: LineModel, t :: Float64)
    ...
    return f.nlp.f(f.x + t*f.d) #obj(f.nlp, f.x + t * f.d)
end
